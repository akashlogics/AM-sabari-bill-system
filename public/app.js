/* ============================================================
   Am.Sabari. VEGETABLES — Billing App
...
/* ---------------- Generic searchable combo widget ----------------
   Used for the New Bill customer picker and every item-row picker.
   Keeps a hidden <input> holding the actual selected id — with the
   same class/id the rest of the code already reads via `.value` —
   plus a visible search box + a filtered dropdown on top of it, so
   nothing else in the file has to change to know about this. */
function setupCombo({ root, hiddenInput, searchInput, listEl, getOptions, onSelect, emptyText }) {
  // Move the dropdown out to <body> so it always renders on top and is
  // never clipped by an ancestor's overflow (e.g. the bill items table's
  // horizontal-scroll wrapper would otherwise cut it off vertically).
  ...
  function renderList(term) {
    ...
    const filtered = t
      ? all.filter(o => o.label.toLowerCase().includes(t) || (o.sub && o.sub.toLowerCase().includes(t)))
      : all;
    listEl.innerHTML = '';
    if (filtered.length === 0) {
      const div = document.createElement('div');
      div.className = 'combo-option combo-empty';
      div.textContent = emptyText || 'எதுவும் கிடைக்கவில்லை';
      listEl.appendChild(div);
    } else {
      filtered.forEach(o => {
        const div = document.createElement('div');
        div.className = 'combo-option';
        div.innerHTML = `${escapeHtml(o.label)}${o.sub ? `<div class="combo-sub">${escapeHtml(o.sub)}</div>` : ''}`;
        div.addEventListener('mousedown', (e) => {
          e.preventDefault(); // keeps focus so blur doesn't hide the list before this click registers
  ...
  searchInput.addEventListener('blur', () => {
    setTimeout(() => {
      listEl.classList.add('hidden');
      // If typing wasn't followed by an actual tap on a result, resync the
      // visible text to whatever is really selected, so it never shows
      // half-typed text next to a stale/mismatched selection.
      const id = hiddenInput.value;
      const match = id ? getOptions().find(o => o.id === id) : null;
      searchInput.value = match ? match.label : '';
    }, 150);
  });
  searchInput.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') { listEl.classList.add('hidden'); searchInput.blur(); }
  });
  ...
  document.addEventListener('click', (e) => {
    if (!root.contains(e.target) && !listEl.contains(e.target)) listEl.classList.add('hidden');
  });
...
const billCustomerCombo = setupCombo({
  root: document.getElementById('billCustomerCombo'),
  hiddenInput: document.getElementById('billCustomerSelect'),
  searchInput: document.getElementById('billCustomerSearchInput'),
  listEl: document.getElementById('billCustomerComboList'),
  getOptions: () => customers.map(c => ({ id: c.id, label: c.name, sub: c.phone || '' })),
  onSelect: () => updatePrevBalanceDisplay(),
  emptyText: 'வாடிக்கையாளர் யாரும் இல்லை'
});

function populateCustomerSelect() {
  // The combo above reads `customers` live on every keystroke, so there's
  // nothing to rebuild — just clear the selection if it pointed at a
  // customer that no longer exists (e.g. was just deleted).
  const hidden = document.getElementById('billCustomerSelect');
  if (hidden.value && !customers.some(c => c.id === hidden.value)) {
...
function clearBillItemRows() {
  ...
  // (see setupCombo) — sweep those away too whenever all rows are wiped
  // at once, or they'd pile up as invisible orphaned nodes.
  document.querySelectorAll('body > .combo-list').forEach(el => {
    if (el.id !== 'billCustomerComboList') el.remove();
  });
...
function addBillRow() {
  ...
  tr.innerHTML = `
    <td class="col-item">
      <div class="combo row-item-combo">
        <input type="hidden" class="row-item-select">
        <input type="text" class="combo-search row-item-search" placeholder="🔍 பொருள் தேடவும்..." autocomplete="off">
  ...
  setupCombo({
    ...
    listEl: itemComboListEl,
    getOptions: () => items.map(it => ({ id: it.id, label: it.name, sub: `${it.unit} — ₹${plainMoney(it.price)}` })),
    onSelect: (id) => {
...
document.getElementById('qcSave').addEventListener('click', async () => {
  ...
  try {
    ...
    populateCustomerSelect();
    billCustomerCombo.setValue(cust.id, cust.name);
    updatePrevBalanceDisplay();
  } catch (err) {
...
function renderLedgerTab() {
  ...
  tbody.querySelectorAll('.ledger-bill-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      switchTab('newbill');
      const c = findCustomer(btn.dataset.id);
      billCustomerCombo.setValue(btn.dataset.id, c ? c.name : '');
      updatePrevBalanceDisplay();
    });
  });
...
document.getElementById('downloadLedgerBtn').addEventListener('click', () => {
  ...
  const aoa = [
    [`Am.Sabari. VEGETABLES — நாள் கணக்கு — ${formatDateDisplay(dateISO)}`],
    [],
    ['பெயர்', 'முதல் பாக்கி', 'பொருள் எடுக்கது', 'மொத்தம்', 'ரூ. கொடுத்தது', 'பாக்கி'],
    ...rows,
...
document.getElementById('downloadMonthlyBtn').addEventListener('click', () => {
  ...
  const aoa = [
    [`Am.Sabari. VEGETABLES — மாத சுருக்கம் — ${monthNameTa(Number(m))} ${y}`],
    [],
...
document.getElementById('cleanupBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    downloadBlob(JSON.stringify(backupData, null, 2), `am-sabari-vegetables-backup-before-cleanup_${todayISO()}.json`, 'application/json');
...
document.getElementById('downloadReportBtn').addEventListener('click', () => {
  ...
  const aoa = [
    [`Am.Sabari. VEGETABLES — விற்பனை அறிக்கை — ${formatDateDisplay(from)} முதல் ${formatDateDisplay(to)} வரை`],
    [],
...
document.getElementById('exportDataBtn').addEventListener('click', () => {
  ...
  downloadBlob(JSON.stringify(data, null, 2), `am-sabari-vegetables-backup_${todayISO()}.json`, 'application/json');
});
